...
from WarehouseApp.app_func import checkAdmin
from WarehouseApp.app_func import send_html_email, uniqueString
...
def user_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'The password you entered is incorrect, check carefully the password you entered')
...
def update_user(request):
    ...
    if request.is_ajax and request.method == 'POST':
        if request.FILES:
            ...
            bio_form.update_biodata_user(user=request.user, update_key=key, update_value=value)
            return JsonResponse({}, status=200)
    ...
    if request.method == 'POST':
        try:
            ...
            if user is not None:
                ...
                messages.error(request, 'The email or password you entered is wrong')
        except:
            messages.error(request, 'The email or password you entered is wrong')
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(email=email_user).exists():
            ...
            while SecretKey.objects.filter(secret_key=secret_reset).exists():
                secret_reset = uniqueString(50)
            secret_form = SecretKeyForm({'email': email_user, 'user_key': secret_reset})
            ...
            send_html_email('Reset Password', content, [email_user], 'send_email/reset_password.html')
            return redirect('userapp:sending-reset')
        else:
            messages.error(request, "The email you entered is not in our system")
    pass_form = SignupForm()
    context = {
        'title': 'Reset Password',
        'pass_form': pass_form,
